...
function aplicarFisicas() {
  ...
  // Detectar el suelo (y=360 porque el canvas mide 400 y el personaje 40)
...
window.addEventListener("keydown", (e) => {
  if (e.code === "Space" && personaje.enSuelo) {
...
function crearObstaculo() {
  ...
  // REGLA MODO NORMAL: Permite hasta 3 balas con una distancia mínima entre ellas
  if (!modoDificil) {
    let ultimo = obstaculos[obstaculos.length - 1];
    if (obstaculos.length < 3 && (!ultimo || canvas.width - ultimo.x > 250)) {
      if (Math.random() < 0.02) {
        obstaculos.push({
          x: canvas.width,
          y: 335, // Un poco más arriba del suelo
          ancho: 30,
          alto: 10,
          color: "#2ed573",
